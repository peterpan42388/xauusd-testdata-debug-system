...
"""v10 orchestrator replay.

Tag: research-replay/round17-orchestrator
Source: SelfUpgrade/17 多方案时段调度系统
"""
...
def _load_base():
    ...
    for p in candidates:
        if p.exists():
            spec = importlib.util.spec_from_file_location('orchestrator_v10_base', str(p))
...
def main():
    ap = argparse.ArgumentParser(description='Run XAUUSD_Survival_Orchestrator_v10 backtest (research-replay round17)')
    ...
    if args.batch == 'single':
        ...
        print('=== Backtest Summary (Orchestrator_v10 / research replay round17) ===')
        print(f'Source      : {src}')
    ...
    csv_path = out_dir / f'orchestrator_v10_backtest_{plan_label}_{args.batch}.csv'
    json_path = out_dir / f'orchestrator_v10_backtest_{plan_label}_{args.batch}_summary.json'
