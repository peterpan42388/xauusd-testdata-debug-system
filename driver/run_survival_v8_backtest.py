...
def _load_cfg():
    obj = json.loads(ROUND8_RESULT.read_text(encoding='utf-8'))
    return _round8.V8Params(**obj['best_params'])


def run_backtest(df: pd.DataFrame):
    p = _load_cfg()
    rs = _round8.simulate(df, p, enable_pyramid=True, collect_details=True)
...
def main():
    ap = argparse.ArgumentParser(description='Run Survival_v8 research-replay backtest (/08 加仓研究口径)')
    ...
    if args.batch == 'single':
        ...
        rs = run_backtest(df)
        print('=== Backtest Summary (Survival_v8 / Round8 research replay) ===')
    ...
    csv_path = out_dir / f'v8_backtest_{args.batch}.csv'
    json_path = out_dir / f'v8_backtest_{args.batch}_summary.json'
