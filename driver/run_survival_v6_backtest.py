...
ROUND3_PY = ROOT / 'SelfUpgrade' / '03 布林角度EMA向量五K结构' / 'round3_boll_ema_5k_research.py'
ROUND3_RESULT = ROOT / 'SelfUpgrade' / '03 布林角度EMA向量五K结构' / 'result_round3.json'
...
_round3 = _load_module(ROUND3_PY, 'round3_research_mod')


def indicators(df: pd.DataFrame) -> pd.DataFrame:
    return _round3.indicators(df)


def _load_cfg():
    obj = json.loads(ROUND3_RESULT.read_text(encoding='utf-8'))
    return _round3.Params(**obj['best_params'])
...
def run_backtest(df: pd.DataFrame):
    ...
    for i in range(35, len(df)):
        ...
        dist = abs(op - sl)
        units = risk_money / dist if dist > 1e-9 else 0
        if units <= 0:
            continue
...
def main():
    ap = argparse.ArgumentParser(description='Run Survival_v6 research-replay backtest (/03 布林EMA五K口径)')
    ...
    if args.batch == 'single':
        ...
        rs = run_backtest(df)
        print('=== Backtest Summary (Survival_v6 / Round3 research replay) ===')
    ...
    csv_path = out_dir / f'v6_backtest_{args.batch}.csv'
    json_path = out_dir / f'v6_backtest_{args.batch}_summary.json'
