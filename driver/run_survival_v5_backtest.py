...
ROUND5_RESULT = ROOT / 'SelfUpgrade' / '05 转移图谱EV结构' / 'result_round5.json'
...
def _load_round5_cfg():
    obj = json.loads(ROUND5_RESULT.read_text(encoding='utf-8'))
    p = obj['best_params']
    return {
        'sl_atr': float(p['sl_atr']),
        'tp_atr': float(p['tp_atr']),
        'max_hold': int(p['max_hold']),
        'long_edges': set(obj['long_edges']),
        'short_edges': set(obj['short_edges']),
    }
