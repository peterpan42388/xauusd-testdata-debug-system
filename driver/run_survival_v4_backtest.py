...
ROUND4_RESULT = ROOT / 'SelfUpgrade' / '04 状态转移条件EV结构' / 'result_round4.json'
...
def _load_round4_cfg():
    obj = json.loads(ROUND4_RESULT.read_text(encoding='utf-8'))
    p = obj['best_params']
    return {
        'sl_atr': float(p['sl_atr']),
        'tp_atr': float(p['tp_atr']),
        'max_hold': int(p['max_hold']),
        'long_states': set(obj['long_states']),
        'short_states': set(obj['short_states']),
    }
